...
model_dir = data_dir + 'model_dic/mouse_mucosa_model.pth'
data = sc.read_h5ad(data_dir + 'mouse_mucosa.h5ad')
adata = sc.read_h5ad(data_dir + 'mouse_mucosa_scDisco.h5ad')
adata.obs['stimulus'] = adata.obs['condition']
...
plt.savefig(data_dir + 'figures/condition/figure11A.png', dpi=600, format='png', bbox_inches='tight')

# sc.pp.neighbors(adata, use_rep='X_latent')
# sc.tl.umap(adata)
# sc.pl.umap(adata, color=['celltype', 'batch', 'age'], title=['UMAP—Bio', 'batch', 'age'])
#
# sc.tl.embedding_density(adata, groupby='age', key_added='age_density')
# sc.pl.umap(adata, color=['celltype', 'age'], ncols=1)
# sc.pl.embedding_density(adata, key='age_density')
...
marker_genes_dict = {
    'interferon alpha': index_double_f[0],
    'none': index_double_f[1],
}
...
plt.savefig(data_dir + 'figures/condition/figure11B.svg', dpi=300, format='svg', bbox_inches='tight')
