...
for i in seed:
    ...
    age = Label_enc.fit_transform(data.obs['condition'])
    meta_cells['condition'] = np.array(age)
    Label_enc = LabelEncoder()
    ...
    adata_inted.write_h5ad(data_dir + 'sampling/' + str(i) + '_' + 'lung_scDisinfact.h5ad')
...
def my_func(data):
    set_seed(2023)
    counts = data.X
    meta_cells = data.obs[['batch', 'age']]
    Label_enc = LabelEncoder()
    condition = Label_enc.fit_transform(data.obs['age'])
    meta_cells['age'] = np.array(condition)
    Label_enc = LabelEncoder()
    batch = Label_enc.fit_transform(data.obs['batch'])
    meta_cells['batch'] = np.array(batch)

    start = time()
    data_dict = scDisInFact.create_scdisinfact_dataset(counts, meta_cells, condition_key=['age'],
                                                       batch_key="batch")
    Ks = [8, 4]
    # training device
    model = scdisinfact(data_dict=data_dict, Ks=Ks, device=device)
...
if __name__ == '__main__':
    ...
    adata_inted.obs['batch'] = np.array(data.obs['batch'])
    adata_inted.obs['celltype'] = np.array(data.obs['celltype'])
    adata_inted.obs['condition'] = np.array(data.obs['age'])

    adata_inted.write_h5ad(data_dir + 'lung_scDisinfact.h5ad')
