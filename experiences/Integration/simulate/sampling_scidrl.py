...
for i in seed:
    ...
    data = pd.DataFrame(adata.X, index=adata.obs_names, dtype='float64')
    data.to_csv(data_dir + 'sampling/' + str(i) + '_' + 'simulate_raw_scidrl.csv')

    df = adata.obs
    df.index = adata.obs_names
    df.to_csv(data_dir + 'sampling/' + str(i) + '_' + 'meta_scidrl.csv')

    np.savez(data_dir + 'sampling/' + str(i) + '_' + 'simulate_raw_scidrl.npz',
             X_latent=data,
