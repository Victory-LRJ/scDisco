...
model_dir = data_dir + 'model_dic/' + 'pancreas_type2_diabetes_model_ae.pth'
aspect_keys = ['disease']

#%% 2. running human pancreas (type2 diabetes) by scDisco-AE
@profile
def my_func(adata):
    set_seed(2023)
    start = time()
...
if __name__ == '__main__':
    set_seed(2023)
    data = sc.read_h5ad(data_dir + 'human_pancreas2.h5ad')
    data.obs['condition'] = data.obs['disease']
    adata = preprocess(data, size_factor=True)
    adata.raw = adata
    adata = my_func(adata)
    adata.write_h5ad(data_dir + "pancreas2_scDisco_ae.h5ad")
