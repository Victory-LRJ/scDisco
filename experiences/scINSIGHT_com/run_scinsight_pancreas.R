setwd("D:/0AALRJ/scRNA-seq/VAE/scDisco/experiences/pancreas")
library(scINSIGHT)
library(SeuratDisk)
library(Seurat)
library(stringr)
...
data_dir = '../../datasets/pancreas/'
# # prepare data
meta = read.csv(paste0(data_dir, "disease.csv"))
row.names(meta)=meta[['X']]

Convert(paste0(data_dir, "human_pancreas.h5ad"), dest="h5seurat",
        assay = "RNA",
        overwrite=F)
seurat_object <- LoadH5Seurat(paste0(data_dir , "human_pancreas.h5seurat"),
...
batch_conditions <- lapply(data.list, function(seurat_obj) {
  seurat_obj@meta.data[["type2_mellitus"]]
...
colnames(mapping)<- c("batch", 'celltype', 'type2_mellitus')
...
clusts = unlist(clust)
write.csv(clusts, paste0(data_dir, "pancreas_scINSIGHT_clust.csv"))


combined_matrix <- do.call(rbind, scinsight_result@norm.W_2)
results = data.frame(combined_matrix, mapping)
write.csv(results, paste0(data_dir, "pancreas_scINSIGHT.csv"))
