...
from model.network import MeanAct, DispAct, buildNetwork
...
class scDisco(nn.Module):
    def __init__(self, input_dim,
        ...
        self.enc_mu1 = nn.Linear(EncLayer[-1], bio_z_dim).to(device)  # mu layer
        self.enc_var1 = nn.Linear(EncLayer[-1], bio_z_dim).to(device)  # logvariance layer
        ...
        self.z_mu_cond = {}
        for aspect in self.aspect_key:
            self.z_mu_cond[aspect] = nn.Linear(EncLayer[-1], cond_dim).to(device)
        self.z_logvar_cond = {}
        for aspect in self.aspect_key:
            self.z_logvar_cond[aspect] = nn.Linear(EncLayer[-1], cond_dim).to(device)

        self.enc_cond = {}
        for aspect in self.aspect_key:
            self.enc_cond[aspect] = nn.Linear(cond_dim, n_conditions[aspect]).to(device)



        self.dec_x = nn.Sequential(nn.Linear(DecLayer[-1], input_dim), nn.ReLU()).to(device)
        self.dec_mean = nn.Sequential(nn.Linear(DecLayer[-1], input_dim), MeanAct())
        self.dec_disp = nn.Sequential(nn.Linear(DecLayer[-1], input_dim), DispAct())
        self.dec_pi = nn.Sequential(nn.Linear(DecLayer[-1], input_dim), nn.Sigmoid())
    ...
    def Encoder_all(self, x, b, c_labels):
        ...
        z_mu_bio = self.enc_mu1(h)
        z_logvar_bio = self.enc_var1(h)
        z_bio = self.reparameterize(z_mu_bio, z_logvar_bio)

        z_mu_cond = {}
        for (aspect, enc) in self.z_mu_cond.items():
            z_mu_cond[aspect] = enc(h)
        z_logvar_cond = {}
        for (aspect, enc) in self.z_logvar_cond.items():
            z_logvar_cond[aspect] = enc(h)
        z_cond = {}
        for aspect in self.aspect_key:
            z_cond[aspect] = self.reparameterize(z_mu_cond[aspect], z_logvar_cond[aspect])
        ...
        return z_mu_bio, z_logvar_bio, z_bio, z_mu_cond, z_logvar_cond, z_cond, z_cond2, z_cond3

    def forward(self, x, b, b_labels):
        z_mu_bio, z_logvar_bio, z_bio, z_mu_cond, z_logvar_cond, z_cond, z_cond2, z_cond3 = self.Encoder_all(x, b, b_labels)
        ...
        return z_mu_bio, z_logvar_bio, z_bio, z_mu_cond, z_logvar_cond, z_cond, x_x, z_cond3
    ...
    def EncodeAll(self, X, B, c_labels, device, batch_size=256):
        ...
        for batch_idx in range(num_batch):
            ...
            with torch.no_grad():
                z_mu_bio, _, _, _, _, _, _, z_cond = self.Encoder_all(exp, b, c_l)

            all_z_bio.append(z_mu_bio.data)
        ...
        return all_z_bio.cpu().numpy(), all_z_cond_

    def fetch_grad(self, x, b, latent_grad, aspect, c_labels):
            set_seed(2023)
            device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")
            x_with_grad = x.requires_grad_(True).type(torch.float).to(device)
            h = self.Encoder(torch.cat([x, b], 1))
            ...
            for aspect in self.aspect_key:
                z2_[aspect] = []
                for i in range(self.n_conditions[aspect]):
                    locals()['index_' + str(i) + aspect] = torch.argwhere(c_labels == i).squeeze()
                    locals()['z2_' + str(i) + aspect] = z_cond[aspect][locals()['index_' + str(i) + aspect]]
                    if locals()['z2_' + str(i) + aspect].dim() == 2:
                        z2_[aspect].append(self.cn_list[aspect][i](locals()['z2_' + str(i) + aspect]))
                    else:
