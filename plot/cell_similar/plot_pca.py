...
# %%
plot_pca_all(Raw, 'celltype', 'Raw', axs[0][0], show=False)
plot_pca_all(Raw, 'batch', 'Raw', axs[1][0], show=False)
plot_pca_all(Raw, 'condition', 'Raw', axs[2][0], show=False)
plot_pca_all(scDisco, 'celltype', 'scDisco', axs[0][1], show=False)
plot_pca_all(scDisco, 'batch', 'scDisco', axs[1][1], show=False)
plot_pca_all(scDisco, 'condition', 'scDisco', axs[2][1], show=False)
plot_pca_all(Harmony, 'celltype', 'Harmony', axs[0][2], show=False)
plot_pca_all(Harmony, 'batch', 'Harmony', axs[1][2], show=False)
plot_pca_all(Harmony, 'condition', 'Harmony', axs[2][2], show=False)
plot_pca_all(scVI, 'celltype', 'scVI', axs[0][3], show=False)
plot_pca_all(scVI, 'batch', 'scVI', axs[1][3], show=False)
plot_pca_all(scVI, 'condition', 'scVI', axs[2][3], show=False)
plot_pca_all(scINSIGHT, 'celltype', 'scINSIGHT', axs[0][4], show=False)
plot_pca_all(scINSIGHT, 'batch', 'scINSIGHT', axs[1][4], show=False)
plot_pca_all(scINSIGHT, 'condition', 'scINSIGHT', axs[2][4], show=False)
plot_pca_all(Seurat, 'celltype', 'Seurat', axs[0][5], show=False)
plot_pca_all(Seurat, 'batch', 'Seurat', axs[1][5], show=False)
plot_pca_all(Seurat, 'condition', 'Seurat', axs[2][5], show=False)
plt.savefig(data_dir + 'figures/pca1.png', dpi=600, format='png')


adata_pca = ad.AnnData(Raw.obsm['X_latent'])
adata_pca.obs['celltype'] = np.array(Raw.obs['celltype'])
adata_pca.obs['batch'] = np.array(Raw.obs['batch'])
adata_pca.obs['condition'] = np.array(Raw.obs['condition'])
sc.tl.pca(adata_pca, n_comps=2, svd_solver='arpack')
sc.pl.pca(adata_pca, color=['celltype'])
plt.savefig(data_dir + 'figures/plot_celltype.png', dpi=600, format='png', bbox_inches='tight')
sc.pl.pca(adata_pca, color=['batch'], palette=col1)
plt.savefig(data_dir + 'figures/plot_batch.png', dpi=600, format='png', bbox_inches='tight')
sc.pl.pca(adata_pca, color=['condition'], palette=col2)
plt.savefig(data_dir + 'figures/plot_condition.png', dpi=600, format='png', bbox_inches='tight')